...
- [1. Introdução à Infraestrutura de IA da NVIDIA](#1-introdução-à-infraestrutura-de-ia-da-nvidia)
  - [1.1. Papel das GPUs em Cargas de Trabalho de IA](#11-papel-das-gpus-em-cargas-de-trabalho-de-ia)
  - [1.2. Arquiteturas CPU vs GPU vs DPU](#12-arquiteturas-cpu-vs-gpu-vs-dpu)
  - [1.3. Aceleração por GPU para Pipelines de IA/ML](#13-aceleração-por-gpu-para-pipelines-de-iaml)
  - [1.4. Visão Geral do Ecossistema NVIDIA (CUDA, Triton, NGC)](#14-visão-geral-do-ecossistema-nvidia-cuda-triton-ngc)
- [2. Gerenciamento de Recursos de GPU e Virtualização](#2-gerenciamento-de-recursos-de-gpu-e-virtualização)
  - [2.1. Configuração MIG (Multi-instance GPU)](#21-configuração-mig-multi-instance-gpu)
  - [2.2. Técnicas de Partilha e Isolamento de GPU](#22-técnicas-de-partilha-e-isolamento-de-gpu)
  - [2.3. Configuração e Casos de Uso de GPUs Virtuais (vGPU)](#23-configuração-e-casos-de-uso-de-gpus-virtuais-vgpu)
  - [2.4. Agendamento de Cargas de Trabalho com GPU no Kubernetes](#24-agendamento-de-cargas-de-trabalho-com-gpu-no-kubernetes)
  - [2.5. Laboratório Prático: Configurar MIG numa A100](#25-laboratório-prático-configurar-mig-numa-a100)
- [3. Armazenamento, Redes e Pipelines de Dados para IA](#3-armazenamento-redes-e-pipelines-de-dados-para-ia)
  - [3.1. Arquiteturas de Armazenamento para Cargas de Trabalho de IA](#31-arquiteturas-de-armazenamento-para-cargas-de-trabalho-de-ia)
  - [3.2. Rede de Alta Velocidade: NVLink, Infiniband, RDMA](#32-rede-de-alta-velocidade-nvlink-infiniband-rdma)
  - [3.3. Gargalos e Otimização no Movimento de Dados](#33-gargalos-e-otimização-no-movimento-de-dados)
  - [3.4. Design de Pipeline de Dados para IA (ETL + Treinamento + Inferência)](#34-design-de-pipeline-de-dados-para-ia-etl--treinamento--inferência)
  - [3.5. Laboratório: Projetar um Pipeline de Dados de Ponta a Ponta para IA](#35-laboratório-projetar-um-pipeline-de-dados-de-ponta-a-ponta-para-ia)
- [4. Orquestração e Escalabilidade de Clusters de IA](#4-orquestração-e-escalabilidade-de-clusters-de-ia)
  - [4.1. Kubernetes para Cargas de Trabalho de IA com GPU](#41-kubernetes-para-cargas-de-trabalho-de-ia-com-gpu)
  - [4.2. Helm, Operators e Autoscaling de Cluster](#42-helm-operators-e-autoscaling-de-cluster)
  - [4.3. Integração de Slurm, Kubeflow e MLflow](#43-integração-de-slurm-kubeflow-e-mlflow)
  - [4.4. Topologias de Cluster (On-prem, Cloud, Híbrido)](#44-topologias-de-cluster-on-prem-cloud-híbrido)
  - [4.5. Laboratório: Deploy de Job de Treinamento Multi-GPU no Kubernetes](#45-laboratório-deploy-de-job-de-treinamento-multi-gpu-no-kubernetes)
- [5. Otimização de Desempenho e Monitoramento](#5-otimização-de-desempenho-e-monitoramento)
  - [5.1. Profiling de Workloads em GPU (Nsight, DLProf, nvtop)](#51-profiling-de-workloads-em-gpu-nsight-dlprof-nvtop)
  - [5.2. Métricas de GPU, Telemetria e Ferramentas de Alertas](#52-métricas-de-gpu-telemetria-e-ferramentas-de-alertas)
  - [5.3. TensorRT e Otimização de Modelos](#53-tensorrt-e-otimização-de-modelos)
  - [5.4. Diagnóstico e Ajuste de Gargalos](#54-diagnóstico-e-ajuste-de-gargalos)
  - [5.5. Laboratório: Otimização de Pipeline de Inferência com TensorRT](#55-laboratório-otimização-de-pipeline-de-inferência-com-tensorrt)
- [6. Segurança, Conformidade e Governança de Dados](#6-segurança-conformidade-e-governança-de-dados)
  - [6.1. Protegendo Workloads Acelerados por GPU](#61-protegendo-workloads-acelerados-por-gpu)
  - [6.2. Criptografia e Controle de Acesso (DPUs, DOCA)](#62-criptografia-e-controle-de-acesso-dpus-doca)
  - [6.3. Controle de Acesso Baseado em Função (RBAC) para Clusters de IA](#63-controle-de-acesso-baseado-em-função-rbac-para-clusters-de-ia)
  - [6.4. Conformidade Regulatória: GDPR, HIPAA, FedRAMP](#64-conformidade-regulatória-gdpr-hipaa-fedramp)
  - [6.5. Laboratório: Aplicar Políticas de Segurança em Infraestrutura de IA](#65-laboratório-aplicar-políticas-de-segurança-em-infraestrutura-de-ia)
- [7. Infraestrutura de IA na Edge e Integração](#7-infraestrutura-de-ia-na-edge-e-integração)
  - [7.1. Edge vs Cloud AI – Implicações de Infraestrutura](#71-edge-vs-cloud-ai--implicações-de-infraestrutura)
  - [7.2. NVIDIA Jetson e Orin para Edge AI](#72-nvidia-jetson-e-orin-para-edge-ai)
  - [7.3. Aprendizado Federado e Inferência Distribuída](#73-aprendizado-federado-e-inferência-distribuída)
  - [7.4. Casos de Uso: Cidades Inteligentes, Varejo e IIoT](#74-casos-de-uso-cidades-inteligentes-varejo-e-iiot)
  - [7.5. Laboratório: Implantar um Modelo de IA no Jetson Nano](#75-laboratório-implantar-um-modelo-de-ia-no-jetson-nano)
- [8. NGC, Triton Inference Server e Implantação](#8-ngc-triton-inference-server-e-implantação)
  - [8.1. Usando o Catálogo NGC para Modelos Pré-treinados](#81-usando-o-catálogo-ngc-para-modelos-pré-treinados)
  - [8.2. Visão Geral e Arquitetura do Triton Inference Server](#82-visão-geral-e-arquitetura-do-triton-inference-server)
  - [8.3. Conjunto de Modelos e Serviço Multi-Framework](#83-conjunto-de-modelos-e-serviço-multi-framework)
  - [8.4. Servindo em Escala – Balanceamento de Carga e Design de Alta Disponibilidade](#84-servindo-em-escala--balanceamento-de-carga-e-design-de-alta-disponibilidade)
  - [8.5. Laboratório: Implantar o Triton com Modelos TensorFlow e ONNX](#85-laboratório-implantar-o-triton-com-modelos-tensorflow-e-onnx)
- [9. Projetos do Mundo Real e Fluxos de Trabalho Empresariais](#9-projetos-do-mundo-real-e-fluxos-de-trabalho-empresariais)
  - [9.1. Estudo de Caso: Construindo um Supercomputador de IA](#91-estudo-de-caso-construindo-um-supercomputador-de-ia)
  - [9.2. Estudo de Caso: Infraestrutura de IA Multi-tenant para Saúde](#92-estudo-de-caso-infraestrutura-de-ia-multi-tenant-para-saúde)
  - [9.3. Fluxo de Trabalho End-to-End: Dados → Treinamento → Implantação → Monitoramento](#93-fluxo-de-trabalho-end-to-end-dados--treinamento--implantação--monitoramento)
  - [9.4. Laboratório: Projetar e Apresentar uma Infraestrutura de IA Escalável](#94-laboratório-projetar-e-apresentar-uma-infraestrutura-de-ia-escalável)
- [10. Projeto Final e Preparação para Certificação](#10-projeto-final-e-preparação-para-certificação)
  - [10.1. Exam Blueprint and Common Pitfalls](#101-exam-blueprint-and-common-pitfalls)
  - [10.2. Flashcards: Concepts, Commands, Tools](#102-flashcards-concepts-commands-tools)
  - [10.3. Capstone Project: End-to-End AI Infrastructure Design](#103-capstone-project-end-to-end-ai-infrastructure-design)
...
### 1.1. Papel das GPUs em Cargas de Trabalho de IA
...
### 1.2. Arquiteturas CPU vs GPU vs DPU
...
### 1.3. Aceleração por GPU para Pipelines de IA/ML
...
### 1.4. Visão Geral do Ecossistema NVIDIA (CUDA, Triton, NGC)
...
## 2. Gerenciamento de Recursos de GPU e Virtualização

### 2.1. Configuração MIG (Multi-instance GPU)
...
### 2.2. Técnicas de Partilha e Isolamento de GPU
...
### 2.3. Configuração e Casos de Uso de GPUs Virtuais (vGPU)
...
### 2.4. Agendamento de Cargas de Trabalho com GPU no Kubernetes
...
### 2.5. Laboratório Prático: Configurar MIG numa A100
...
## 3. Armazenamento, Redes e Pipelines de Dados para IA

### 3.1. Arquiteturas de Armazenamento para Cargas de Trabalho de IA
...
### 3.2. Rede de Alta Velocidade: NVLink, Infiniband, RDMA
...
### 3.3. Gargalos e Otimização no Movimento de Dados
...
### 3.4. Design de Pipeline de Dados para IA (ETL + Treinamento + Inferência)
...
### 3.5. Laboratório: Projetar um Pipeline de Dados de Ponta a Ponta para IA
...
## 4. Orquestração e Escalabilidade de Clusters de IA

### 4.1. Kubernetes para Cargas de Trabalho de IA com GPU
...
### 4.2. Helm, Operators e Autoscaling de Cluster
...
### 4.3. Integração de Slurm, Kubeflow e MLflow
...
### 4.4. Topologias de Cluster (On-prem, Cloud, Híbrido)
...
### 4.5. Laboratório: Deploy de Job de Treinamento Multi-GPU no Kubernetes
...
### 5.1. Profiling de Workloads em GPU (Nsight, DLProf, nvtop)
...
### 5.2. Métricas de GPU, Telemetria e Ferramentas de Alertas
...
### 5.3. TensorRT e Otimização de Modelos
...
### 5.4. Diagnóstico e Ajuste de Gargalos
...
### 5.5. Laboratório: Otimização de Pipeline de Inferência com TensorRT
...
## 6. Segurança, Conformidade e Governança de Dados

### 6.1. Protegendo Workloads Acelerados por GPU
...
### 6.2. Criptografia e Controle de Acesso (DPUs, DOCA)
...
### 6.3. Controle de Acesso Baseado em Função (RBAC) para Clusters de IA
...
### 6.4. Conformidade Regulatória: GDPR, HIPAA, FedRAMP
...
### 6.5. Laboratório: Aplicar Políticas de Segurança em Infraestrutura de IA
...
## 7. Infraestrutura de IA na Edge e Integração

### 7.1. Edge vs Cloud AI – Implicações de Infraestrutura
...
### 7.2. NVIDIA Jetson e Orin para Edge AI
...
### 7.3. Aprendizado Federado e Inferência Distribuída
...
### 7.4. Casos de Uso: Cidades Inteligentes, Varejo e IIoT
...
### 7.5. Laboratório: Implantar um Modelo de IA no Jetson Nano
...
## 8. NGC, Triton Inference Server e Implantação

### 8.1. Usando o Catálogo NGC para Modelos Pré-treinados
...
### 8.2. Visão Geral e Arquitetura do Triton Inference Server
...
### 8.3. Conjunto de Modelos e Serviço Multi-Framework
...
### 8.4. Servindo em Escala – Balanceamento de Carga e Design de Alta Disponibilidade
...
### 8.5. Laboratório: Implantar o Triton com Modelos TensorFlow e ONNX
...
## 9. Projetos do Mundo Real e Fluxos de Trabalho Empresariais

### 9.1. Estudo de Caso: Construindo um Supercomputador de IA
...
### 9.2. Estudo de Caso: Infraestrutura de IA Multi-tenant para Saúde
...
### 9.3. Fluxo de Trabalho End-to-End: Dados → Treinamento → Implantação → Monitoramento
...
## 9.4. Laboratório: Projetar e Apresentar uma Infraestrutura de IA Escalável
...
## 10. Projeto Final e Preparação para Certificação

### 10.1. Exam Blueprint and Common Pitfalls
...
### 10.2. Flashcards: Concepts, Commands, Tools
...
### 10.3. Capstone Project: End-to-End AI Infrastructure Design
...
### 10.4. Certification Pathways and Next Steps
